#! /usr/bin/env python
# Nodo para la prueba: Navegacion CON obstaculos estaticos
...
def callback_L(data1):
	...
	x_list = np.count_nonzero(np.where((pc['x']>=-1.0)&(pc['x']<=1.0)))
	z_list = np.count_nonzero(np.where((pc['z']>=-10.0)&(pc['z']<=-0.0)))
	if (x_list>=2740) and (z_list>=13800): Obj = True 
	else: Obj = False
	if (Obj==True): 
		Ev = True
		FT = 0
