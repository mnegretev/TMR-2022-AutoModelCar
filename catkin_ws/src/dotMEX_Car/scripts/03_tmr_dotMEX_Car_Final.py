#! /usr/bin/env python
# Nodo para la prueba: Navegacion CON obstaculos dinamicos
...
def callback_L(data1):
	global Ev,Obj, FT, k, FTE
	pc = ros_numpy.numpify(data1)
	x_list = np.count_nonzero((pc['x']>=-1.0)&(pc['x']<=1.0)) 
	z_list = np.count_nonzero((pc['z']>=-10.0)&(pc['z']<=-0.0))

	#print(z_list)
	if (x_list>=2740) and (z_list>=13800): Obj = True #2740
	else: Obj = False
	if (Obj==True): 
		Ev = True
		FT = 0
...
def callback_V(data0):
	...
	imagenF = cv2.Sobel(imagenF, cv2.CV_8U, 1, 0, ksize=9, borderType=cv2.BORDER_DEFAULT)
	y1 = 0
	y2 = 0
	if (Ev==False):	
		side = 1
...
if __name__ == '__main__':
	...
	print("Nodo inicializado: TMR_03.py")
	rospy.init_node('TMR_03',anonymous=True)												
